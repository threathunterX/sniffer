...
class RabbitmqConsumer(object):
    def __init__(self, amqp_url, queue_name, exchange_name, exchange_type, durable, routing_key, msg_callback):
    ...
    def rabbitmq_callback(self, body, message):
        message.ack()
        print 23333, body
...
class RabbitmqDriver(Driver):
    ...
    def _recv_msg_fn(self, msg, addr='rabbitmq'):
        for m in msg.split("\n"):
            self._recv_msg_fn_in(m, addr)

    def _recv_msg_fn_in(self, msg, addr='rabbitmq'):
        """
        nginx format:
        ...
        try:
            ...
            self.logger.debug("get rabbitmq msg %s from address %s", msg, addr)
            # remove the header
            if "nginx:" in msg:
            ...
            self.logger.debug("get http data from rabbitmq: %s", args)
            try:
            ...
            self.logger.debug("get http msg from rabbitmq: %s", new_msg)
            self.put_msg(new_msg)

            print 1111, new_msg
            return new_msg
        except Exception as ex:
            self.logger.error("fail to parse rabbitmq data: %s", ex)
    ...
    def add_data_metrics(self, source, data_type, subtype=""):
        tags = {"source_type": "rabbitmq", "source": source, "type": data_type, "subtype": subtype}
    ...
    def start(self):
        self.logger.info("start rabbitmq driver")
